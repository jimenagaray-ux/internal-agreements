...
export function CreateAgreementFlowImproved({ onBack, onNavigateToInternalAgreements, preselectedType }: CreateAgreementFlowImprovedProps) {
  ...
  const openRatesMatrix = (scaleId: string, scaleName: string) => {
    ...
    try {
      ...
      if (!paymentMethods || !liberationDays) {
        console.error('Payment methods or liberation days not defined')
        return
      }

      // Inicializar matriz inmediatamente si no existe
      setRatesMatrix(prev => {
        if (!prev[scaleId]) {
      ...
      // Inicializar errores inmediatamente si no existen
      ...
      // Abrir el modal después de inicializar los datos
      setRatesMatrixPopup({
        isOpen: true,
        scaleId,
  ...
  const updateRateInMatrix = (scaleId: string, paymentMethod: string, liberationDay: string, rate: string) => {
    try {
      // Validar parámetros
      if (!scaleId || !paymentMethod || !liberationDay) {
        console.error('Invalid parameters for updateRateInMatrix:', { scaleId, paymentMethod, liberationDay })
        return
      }
  ...
  const validateScaleMatrix = (scaleId: string) => {
    try {
      if (!scaleId) {
        console.error('Invalid scaleId for validation')
        return { hasErrors: false, filledCount: 0 }
      }
      ...
      console.error('Error validating scale matrix:', error)
      return { hasErrors: false, filledCount: 0 }
    }
  }
  ...
  const getRatesSummary = (scaleId: string) => {
    try {
      if (!scaleId) return 'Configurar tasas'
      ...
      // Contar tasas llenas y verificar errores
      Object.values(matrix).forEach(pm => {
        if (pm && typeof pm === 'object') {
          Object.values(pm).forEach(rate => {
      ...
      if (errors && typeof errors === 'object') {
        Object.values(errors).forEach(pm => {
          if (pm && typeof pm === 'object') {
            Object.values(pm).forEach(error => {
      ...
      if (filledRates === 0) return 'Configurar tasas'
      
      if (hasErrors) {
        return `${filledRates} tasas (con errores)`
      }
      
      return `${filledRates} tasas configuradas`
    } catch (error) {
      console.error('Error getting rates summary:', error)
      return 'Configurar tasas'
    }
  }
                              ...
                              <DialogFooter className="flex flex-col gap-3">
                                ...
                                <div className="flex gap-3 justify-end">
                                  ...
                                  <MPButton 
                                    ...
                                    disabled={(() => {
                                      try {
                                        if (!ratesMatrixPopup.scaleId) return true
                                        const validation = validateScaleMatrix(ratesMatrixPopup.scaleId)
